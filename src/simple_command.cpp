 ...
 * file descriptors
 */
void SimpleCommand::launch(
  const int & fdin, const int & fdout,
...
void SimpleCommand::setup_process_io(
  const int & fdin,
  const int & fdout,
  const int & fderr)
{
...
void SimpleCommand::resume_io(
  const int & saved_fdin,
...
void SimpleCommand::handle_printenv()
{
  if (arguments[0] == std::string("printenv")) {
    char ** _env = environ;
    for (; *_env; ++_env) {
      std::cout << *_env << std::endl;
    }
    _exit(0);
  }
}

bool SimpleCommand::handle_setenv(
  const int & fdin,
  const int & fdout,
  const int & fderr)
{
...
bool SimpleCommand::handle_unsetenv(
  const int & fdin,
  const int & fdout,
  const int & fderr)
{
